...
case class Event[A: Show: Eq](msg: A, timestamp: Long) {
...
object Event {
  ...
  implicit def order[B]: Ordering[Event[B]] =
    (x: Event[B], y: Event[B]) =>
      implicitly[Ordering[Long]].compare(x.timestamp, y.timestamp)

  implicit def eq[A]: Eq[Event[A]] = Eq.fromUniversalEquals
